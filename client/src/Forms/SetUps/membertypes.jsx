...
class membertypes extends Component {
    ...
    exportpdf = () => {
        ...
        const rows = [...this.state.membertypes];

        var doc = new jsPDF("p", "pt");
        doc.autoTable(columns, rows, {
            margin: { top: 60 },
            beforePageContent: function (data) {
                doc.text("ARCM MEMBER TYPES", 40, 50);
            }
        });
        doc.save("ARCM Member types.pdf");
    };
    ProtectRoute() {
        ...
        fetch("/api/membertypes", {
            ...
            .then(membertypes => {
                if (membertypes.length > 0) {
                    this.setState({ membertypes: membertypes });
                } else {
                    swal("", membertypes.message, "error");
                }
            })
            .catch(err => {
    ...
    handleSubmit = event => {
        ...
        if (this.state.isUpdate) {
            this.UpdateData("/api/membertypes/" + this.state.Code, data);
        } else {
            this.postData("/api/membertypes", data);
        }
    };
    handleEdit = d => {
    ...
    handleDelete = k => {
        swal({
            ...
            if (willDelete) {
                return fetch("/api/membertypes/" + k, {
                    method: "Delete",
                    headers: {
                        "Content-Type": "application/json",
                        "x-access-token": localStorage.getItem("token")
    ...
    render() {
        ...
        if (rows.length > 0) {
            rows.forEach(k => {
                const Rowdata = {
                    ...
                    action: (
                        <span>
                            {this.validaterole("Member types", "Edit") ? (
                                <a
                            ...
                            &nbsp;
              {this.validaterole("Member types", "Remove") ? (
                                <a
        ...
        return (
            <div>
                    ...
                    <div className="row wrapper border-bottom white-bg page-heading">
                        ...
                        <div className="col-lg-3">
                            <div className="row wrapper ">
                                {this.validaterole("Member types", "AddNew") ? (
                                    <button
                                        type="button"
                                        style={{ marginTop: 40 }}
                ...
                &nbsp;
                                {this.validaterole("Member types", "Export") ? (
                                    <button
                ...
                {this.validaterole("Member types", "Export") ? (
                                    ...
                                    >
                                        <ExcelSheet data={rows} name="Member types">
