...
class DecisionsApproval extends Component {
  ...
  SendMail = (Name, email, ID, subject, ApplicationNo, Remarks) => {
    const emaildata = {
      to: email,
      subject: subject,
      ID: ID,
      Name: Name,
      ApplicationNo: ApplicationNo, Remarks: Remarks
    };
  ...
  sendBulkNtification = (AproverEmail, AproverMobile, Name, ApplicationNo,Remarks) => {
    this.SendSMS(
      AproverMobile,
      "Decision for ApplicationNo:" +
        ApplicationNo +
        " has been Declined, Edit and resubmit for review."
    );
    this.SendMail(
      Name,
      AproverEmail,
      "DECISION REPORT DECLINED",
      "DECISION  APPROVAL",
      ApplicationNo,
      Remarks
    );
  ...
  handleSubmit = event => {
    ...
    fetch("/api/decisiondocuments/ApproveDecision", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-access-token": localStorage.getItem("token")
      ...
      .then(response =>
        response.json().then(data => {
          if (data.success) {
            ...
            this.setState({ Opendecline:false,summary: false });
          } else {
  ...
  handleDecline = event => {
    ...
    const data = {
      ApplicationNo: this.state.ApplicationNo,
      Declineremarks: this.state.Declineremarks
    };
        ...
        .then(response =>
          response.json().then(data => {
            if (data.success) {
              ...
              this.setState({ Declineremarks: "", Opendecline: false, summary: false });
              this.fetchApplications();
            } else {
              swal("", data.message, "error");
            }
      ...
      return (
        <div>
          ...
          <div className="row wrapper border-bottom white-bg page-heading">
            ...
            <Modal
              visible={this.state.Opendecline}
              width="60%"
              height="290px"
              effect="fadeInUp"
            >
                      ...
                      <form onSubmit={this.handleDecline}>
                        ...
                        <div classname="row">
                          <div class="col-sm-12">
                            <textarea
                              onChange={this.handleInputChange}
                              value={this.state.Declineremarks}
                              type="text"
                              required
                              name="Declineremarks"
                              className="form-control"
